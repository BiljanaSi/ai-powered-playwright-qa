...
test.describe('Login Functionality Test Suite', () => {
  test('@regression TC-009: Case Sensitivity Test - Uppercase Username', async ({ page }) => {
    const loginPage = new LoginPage(page);

    // Navigate to https://www.saucedemo.com/
    await loginPage.open();

    // Attempt login with uppercase username (case sensitivity test)
    await loginPage.login('STANDARD_USER', 'secret_sauce');

    // Verify error message displayed (case-sensitive username)
